...
// ── Tickets thermiques (80mm / 58mm / 50mm) — imprimantes POS / Bluetooth ─────

function thermalTicketCSS(widthMm: number) {
  const base = widthMm >= 80 ? 12 : 10
  return `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    html, body { width: ${widthMm}mm; }
    body { font-family: 'Courier New', Courier, monospace; font-size: ${base}px; color: #000; padding: 2mm; }
    ...
    .big { font-size: ${base + 3}px; }
    .small { font-size: ${base - 2}px; color: #333; }
...
/**
 * Ticket compact pour imprimante thermique (POS/Bluetooth). Réutilise les
 * mêmes données qu'un BL/facture A4, mise en page monospace étroite.
 */
export function printTicketThermal(bl: BonLivraison, widthMm: 80 | 58 | 50, company?: CompanyConfig) {
  const cfg = { ...EMPIRE_FRESH_CONFIG, ...company }
  const companyNom = cfg.nom ?? "Dima Krib Lik"
  const dateStr = bl.date ?? new Date().toLocaleDateString("fr-FR")
  const blId = (bl as unknown as { numero?: string }).numero ?? bl.id
  const livreurNom = (bl as unknown as { livreurNom?: string }).livreurNom
  const montantCaisses = (bl as unknown as { montantCaisses?: number }).montantCaisses ?? 0
  const montantTTC = (bl as unknown as { montantTTC?: number }).montantTTC ?? bl.montantTotal
  const subtotal = bl.lignes.reduce((s, l) => s + l.total, 0)
  const totalTTC = montantTTC + montantCaisses

  const html = `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8" />
<title>Ticket ${blId}</title>
<style>${thermalTicketCSS(widthMm)}</style>
</head>
<body>
  <div class="center bold big">${companyNom}</div>
  ${cfg.adresse ? `<div class="center small">${cfg.adresse}</div>` : ""}
  ${cfg.telephone ? `<div class="center small">Tel: ${cfg.telephone}</div>` : ""}
  ${cfg.ice ? `<div class="center small">ICE: ${cfg.ice}</div>` : ""}
  <div class="line"></div>
  <div class="row"><span>BL ${String(blId).slice(0, 10)}</span><span>${dateStr}</span></div>
  <div>Client: ${bl.clientNom}</div>
  ${livreurNom ? `<div class="small">Livreur: ${livreurNom}</div>` : ""}
  <div class="line"></div>
  <table>
    ${bl.lignes.map(l => `
      <tr><td colspan="2">${l.articleNom}</td></tr>
      <tr>
        <td class="small">${l.quantite} x ${l.total !== undefined ? (l.total / (l.quantite || 1)).toLocaleString("fr-MA", { minimumFractionDigits: 2 }) : "0.00"}</td>
        <td class="right">${l.total.toLocaleString("fr-MA", { minimumFractionDigits: 2 })}</td>
      </tr>`).join("")}
  </table>
  <div class="line"></div>
  <div class="row"><span>Sous-total</span><span>${subtotal.toLocaleString("fr-MA", { minimumFractionDigits: 2 })} DH</span></div>
  ${montantCaisses > 0 ? `<div class="row"><span>Caisses</span><span>${montantCaisses.toLocaleString("fr-MA", { minimumFractionDigits: 2 })} DH</span></div>` : ""}
  <div class="line"></div>
  <div class="row bold big"><span>TOTAL</span><span>${totalTTC.toLocaleString("fr-MA", { minimumFractionDigits: 2 })} DH</span></div>
  <div class="line"></div>
  <div class="center small" style="margin-top:6px">${cfg.mentionsBL ?? "Merci de votre confiance"}</div>
  <div class="center small">${new Date().toLocaleString("fr-MA")}</div>
  <script>window.onload=()=>{window.print();}</script>
</body>
</html>`

  const w = window.open("", "_blank", `width=${widthMm >= 80 ? 400 : 320},height=700`)
  if (w) { w.document.write(html); w.document.close() }
}

export function printTicket80mm(bl: BonLivraison, company?: CompanyConfig) {
  printTicketThermal(bl, 80, company)
}

export function printTicket58mm(bl: BonLivraison, company?: CompanyConfig) {
  printTicketThermal(bl, 58, company)
}

export function printTicket50mm(bl: BonLivraison, company?: CompanyConfig) {
  printTicketThermal(bl, 50, company)
}
